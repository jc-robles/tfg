...
def HarmoicRatio(sensordata, locs_peaks, Fsamp, Is_AP):
    locHR = locs_peaks
    if len(locHR) % 2 == 0:
        locHR = np.delete(locHR,
                          [0, 1, len(locHR) - 2, len(locHR) - 1])  # remove the first two steps and the last two steps
    else:
...
JerkSw, gait_sym, step_reg_even, step_reg_odd, step_reg, St_tv, Cadence, numSteps, HR_AP, HR_ML = two_min_walk(accx,
                                                                                                               accy,
                                                                                                               accz, Fs)
result = {
    "Jerk": str(JerkSw),
    "Gait symmetric": str(gait_sym),
    "Regularity of even steps": str(step_reg_even),
    "Regularity of odd steps": str(step_reg_odd),
    "Regularity of steps": str(step_reg),
    "Step velocity": str(St_tv),
    "Cadence": str(Cadence),
    "Number of steps": str(numSteps),
    "Harmonic ratio antero-posterior": HR_AP.tolist(),
    "Harmonic ratio medio-lateral": HR_ML.tolist()
}
